...
import { AmazonDataMapper } from './AmazonDataMapper';
import {
  AmazonErrorCode,
  AmazonTokenResponse,
  AmazonUserData,
 ...
 */
const AMAZON_PROVIDER_CONFIG: OAuthProviderInterface = {
  id: 'amazon',
  name: 'amazon',
  displayName: 'Amazon Prime',
  logoUrl: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/amazonwebservices-original.svg',
  type: OAuthProviderType.LWA, // Login with Amazon
...
/**
 * Amazon-specific error codes and handling
 */
enum AmazonErrorCodeEnum {
...
/**
 * Amazon OAuth Provider Implementation
 *
 * Handles Amazon-specific OAuth flows, token management, and API interactions
 */
export class AmazonProvider extends OAuthProvider {
  private clientId: string;
  private clientSecret: string;
  private redirectUri: string;
  private apiClient: AmazonApiClient;
  private dataMapper: typeof AmazonDataMapper;

 constructor(clientId: string, clientSecret: string, redirectUri: string) {
    super(AMAZON_PROVIDER_CONFIG);
    ...
    this.dataMapper = AmazonDataMapper;
  }

  /**
   * Get authorization URL for Amazon OAuth flow
   */
  ...
  async revokeTokens(accessToken: string, refreshToken?: string): Promise<boolean> {
    try {
      // Amazon doesn't have a standard token revocation endpoint
      // We'll return true for now as there's no way to revoke tokens
      console.warn('Amazon does not support token revocation via public API');
      return true;

    } catch (error) {
      console.error('Failed to revoke Amazon tokens:', error);
      return false;
    }
  }
  ...
  async getMappedSubscriptions(accessToken: string): Promise<OAuthSubscription[]> {
    try {
      ...
      return this.dataMapper.mapMultipleSubscriptions(subscriptions as any, {
        ...
        postal_code: userProfile.metadata?.postalCode,
  ...
  /**
   * Sync Amazon subscription data to internal format
   */
  async syncSubscriptionData(accessToken: string): Promise<AmazonSyncResult> {
    ...
    try {
        ...
        try {
          ...
          const mappedBilling = this.dataMapper.mapToInternalBilling(
            billingRecords.map(record => ({
              ...
              status: record.metadata?.status?.toUpperCase() || 'SUCCESS',
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      result.errors.push(`Failed to sync Amazon data: ${errorMessage}`);
   ...
   */
  getDataMapper(): typeof AmazonDataMapper {
    return this.dataMapper;
  }

  /**
   * Create Amazon-specific OAuth error
   */
  private createAmazonError(statusCode: string, errorData: any): OAuthError {
    let errorCode = AmazonErrorCodeEnum.SERVER_ERROR;
    let errorMessage = 'Unknown Amazon API error';

    // Map HTTP status codes to Amazon error codes
    switch (statusCode) {
      case '400':
        errorCode = AmazonErrorCodeEnum.INVALID_REQUEST;
        errorMessage = 'Invalid request parameters';
        break;
      case '401':
        errorCode = AmazonErrorCodeEnum.INVALID_CLIENT;
        errorMessage = 'Invalid client credentials';
        break;
      case '403':
        errorCode = AmazonErrorCodeEnum.ACCESS_DENIED;
        errorMessage = 'Access denied';
        break;
      case '429':
        errorCode = AmazonErrorCodeEnum.RATE_LIMIT_EXCEEDED;
        errorMessage = 'Rate limit exceeded';
        break;
      case '500':
        errorCode = AmazonErrorCodeEnum.SERVER_ERROR;
        errorMessage = 'Amazon server error';
        break;
      case '503':
        errorCode = AmazonErrorCodeEnum.TEMPORARILY_UNAVAILABLE;
  ...
  /**
   * Parse Amazon token response
   */
  protected parseTokenResponse(tokenData: AmazonTokenResponse): OAuthTokens {
   ...
   */
  getConfig(): AmazonProviderConfig {
    return {
      ...
      apiClient: {
        ...
        timeout: this.apiClient.getConfig().timeout || 3000,
  ...
  /**
   * Get supported Amazon API versions
   */
  ...
  updateApiClientConfig(config: {
    ...
    if (config.timeout || config.maxRetries || config.retryDelay) {
      // Note: This would require extending AmazonApiClient to support runtime config updates
