...
export const syncBills = async (bills: Bill[]): Promise<void> => {
    await initDB();
    return new Promise((resolve, reject) => {
        const store = getStore(BILLS_STORE, 'readwrite');
        store.clear().onsuccess = () => {
            bills.forEach(bill => store.put(bill));
            resolve();
        };
        store.transaction.onerror = () => reject('Error syncing bills');
    });
}
...
export const syncSubscriptions = async (subs: Subscription[]): Promise<void> => {
    await initDB();
    return new Promise((resolve, reject) => {
        const store = getStore(SUBS_STORE, 'readwrite');
        store.clear().onsuccess = () => {
            subs.forEach(sub => store.put(sub));
            resolve();
        };
        store.transaction.onerror = () => reject('Error syncing subscriptions');
    });
}
